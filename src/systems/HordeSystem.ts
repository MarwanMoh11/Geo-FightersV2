...
const CONFIG = {
    ...
    enemyWeights: {
        low: { [EnemyType.VIRUS]: 0.75, [EnemyType.GLITCH]: 0.25, [EnemyType.FIREWALL]: 0.00 },
        optimal: { [EnemyType.VIRUS]: 0.65, [EnemyType.GLITCH]: 0.32, [EnemyType.FIREWALL]: 0.03 },
        high: { [EnemyType.VIRUS]: 0.55, [EnemyType.GLITCH]: 0.40, [EnemyType.FIREWALL]: 0.05 },
    },
...
function spawnSingleEnemy(playerPos: THREE.Vector3, scene: THREE.Scene, zone: 'low' | 'optimal' | 'high'): void {
...
function spawnWave(playerPos: THREE.Vector3, scene: THREE.Scene, zone: 'low' | 'optimal' | 'high'): void {
    const waveSize = CONFIG.waveSize.min + Math.floor(Math.random() * (CONFIG.waveSize.max - CONFIG.waveSize.min + 1));
    ...
    for (let i = 0; i < waveSize; i++) {
        ...
        const distance = CONFIG.minSpawnDistance + Math.random() * (CONFIG.maxSpawnDistance - CONFIG.minSpawnDistance);
...
function calculateSpawnPosition(playerPos: THREE.Vector3, zone: 'low' | 'optimal' | 'high'): { x: number; z: number } {
