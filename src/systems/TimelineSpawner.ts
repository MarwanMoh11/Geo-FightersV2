...
export function TimelineSpawnerSystem(dt: number, scene: THREE.Scene): void {
    ...
    if (spawnTimer <= 0) {
        ...
        let spawnBudget = CONFIG.BASE_BUDGET +
            Math.pow(minutesElapsed, CONFIG.BUDGET_EXPONENT) * CONFIG.BUDGET_SCALE;
...
function spawnFromBudget(
    ...
    while (remainingBudget > 0 && spawnedThisCycle < maxSpawnsPerCycle) {
        ...
        const entryCount = entry.countMin + Math.floor(
            Math.random() * (entry.countMax - entry.countMin + 1)
        );
    ...
    switch (formation) {
        ...
        case 'pincer':
            ...
            for (let i = 0; i < count; i++) {
                ...
                const offset = countPerArc > 1
                    ? (indexInArc / (countPerArc - 1) - 0.5) * arcSpread
                    : 0;
...
function randomSpawnDistance(): number {
    return CONFIG.INNER_SPAWN_RADIUS +
...
export function getSpawnerDebugInfo(): {
    ...
    const minutesElapsed = gameTime / 60;
    const currentBudget = CONFIG.BASE_BUDGET +
        Math.pow(minutesElapsed, CONFIG.BUDGET_EXPONENT) * CONFIG.BUDGET_SCALE;
